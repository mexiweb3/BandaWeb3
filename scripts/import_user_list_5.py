...
def main():
    ...
    txt_path = Path("website/inputs/hosted_spaces_4.txt")
    ...
    for i, line in enumerate(lines):
        if "Ended:" in line:
            ...
            # Handle potential same-day duplicates (multiple "Platicame de tu proyecto" on Mar 4)
            # If title is SAME, we skip? 
            # Wait, "Platicame de tu proyecto" appears twice on Mar 4 with diff duration.
            # Normalization catches the first one, skips second.
            # Should we add suffix?
            # If norm title exists, AND date matches... 
            # Actually norm_titles Logic skips simplisticly.
    ...
    print(f"Found {len(new_eps)} new 'Hosted Spaces' (Batch 4) to add.")
    ...
    if new_eps:
        ...
        print("Database updated with hosted spaces batch 4.")
